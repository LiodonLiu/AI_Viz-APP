...
class BingImages():
    ...
    def download(urls,name, folder="."):
        ...
        for link in urls:
            try:
                link = link.replace("\\","/")
                imgtype = link[len(link) - 1 - link[::-1].index("/"):]
                if "?" in imgtype:
                    imgtype = imgtype[:len(imgtype) - 1 - imgtype[::-1].index("?")]
                imgtype = imgtype[-6:]
                ...
                with open(folder + "/" + imgName+'.'+imgtype, 'wb') as f:
...
if __name__ == "__main__":
    ...
    BingImages.download(musk,"Elon Musk","./tmp")